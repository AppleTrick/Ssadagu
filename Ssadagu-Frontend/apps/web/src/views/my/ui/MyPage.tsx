      ...
      <ContentArea>
        <ProfileSection>
          ...
          {!isLoading && !isError && user && (
            <ProfileHeader user={user} onEditClick={() => router.push(ROUTES.MY_PROFILE)} />
          )}
        </ProfileSection>

        <QuickMenuRow>
          <QuickMenuItem icon={<SellIcon />} label="판매 내역" onClick={() => router.push(ROUTES.MY_PRODUCTS)} />
          <QuickMenuItem icon={<BuyIcon />} label="구매 내역" onClick={() => router.push(ROUTES.MY_TRANSACTIONS)} />
          <QuickMenuItem icon={<WishIcon />} label="관심 목록" onClick={() => router.push(ROUTES.MY_WISHES)} />
        </QuickMenuRow>

        <MenuGroup>
          <MenuListItem label="동네 재인증" onClick={() => router.push(ROUTES.MY_ACCOUNT)} />
          <MenuListItem label="알림 설정" onClick={() => {}} />
          <MenuListItem label="로그아웃" onClick={() => setShowLogoutDialog(true)} />
          <MenuListItem label="탈퇴하기" color="danger" onClick={() => setShowWithdrawDialog(true)} />
        </MenuGroup>
      </ContentArea>
