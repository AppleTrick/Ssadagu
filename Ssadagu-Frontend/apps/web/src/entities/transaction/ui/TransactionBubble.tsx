...
const TransactionBubble = ({
  ...
  const rawType =
    (message as any).type || message.messageType || "PAYMENT_REQUEST";
  const resolvedType = (message as any).resolvedType; // ChatRoomPage에서 주입한 상태

  // 이미 성공/실패로 처리된 요청이거나, 메시지 자체가 결과 메시지인 경우
  const isSuccess =
    rawType === "PAYMENT_SUCCESS" || resolvedType === "PAYMENT_SUCCESS";
  ...
  return (
    <Container $isMyMessage={isMyMessage} $isSuccess={isSuccess}>
      <Header $isMyMessage={isMyMessage} $isSuccess={isSuccess}>
        {headerText}
      </Header>
      ...
      <CardBody>
        ...
        <Info>
          ...
          <Price $isMyMessage={isMyMessage}>
            {price?.toLocaleString() || 0}원
          </Price>
          <Details
            $isMyMessage={isMyMessage}
            style={{ fontSize: "10px", marginTop: "6px", opacity: 0.8 }}
          >
            구매자: {contentData.buyerNickname || "-"} / 판매자:{" "}
            {contentData.sellerNickname || "-"}
          </Details>
        </Info>
      </CardBody>
...
const Container = styled.div<{ $isMyMessage: boolean; $isSuccess?: boolean }>`
  ...
  border-top-right-radius: ${({ $isMyMessage }) =>
    $isMyMessage ? "4px" : "18px"};
  border-top-left-radius: ${({ $isMyMessage }) =>
    $isMyMessage ? "18px" : "4px"};
  ...
  border: ${({ $isSuccess }) =>
    $isSuccess ? `1px solid ${colors.primary}` : "none"};
`;
...
const Header = styled.div<{ $isMyMessage: boolean; $isSuccess?: boolean }>`
  ...
  border-bottom: 1px solid
    ${({ $isMyMessage }) =>
      $isMyMessage ? "rgba(255, 255, 255, 0.2)" : colors.border};
`;
...
const ItemTitle = styled.div<{ $isMyMessage: boolean }>`
  ...
  color: ${({ $isMyMessage }) =>
    $isMyMessage ? "rgba(255, 255, 255, 0.8)" : colors.textSecondary};
  white-space: nowrap;
  overflow: hidden;
...
const Price = styled.div<{ $isMyMessage: boolean }>`
  ...
  font-weight: ${typography.weight.bold};
  color: ${({ $isMyMessage }) =>
    $isMyMessage ? colors.surface : colors.textPrimary};
`;
...
const Details = styled.div<{ $isMyMessage: boolean }>`
  ...
  font-size: ${typography.size.xs};
  color: ${({ $isMyMessage }) =>
    $isMyMessage ? "rgba(255, 255, 255, 0.7)" : colors.textSecondary};
  margin-top: 2px;
`;
...
const Button = styled.button<{ $isMyMessage: boolean }>`
  ...
  background: ${({ $isMyMessage }) =>
    $isMyMessage ? "rgba(255, 255, 255, 0.15)" : "#E9ECEF"};
  border: none;
  border-radius: 12px;
