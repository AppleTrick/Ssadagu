...
const MessagesArea = styled.div`
  flex: 1;
  overflow-y: auto;
  padding: ${HEADER_HEIGHT + 80}px 16px ${CHAT_INPUT_HEIGHT + 12}px;
  display: flex;
  flex-direction: column;
  gap: 8px;
`;
...
export default function ChatRoomPage() {
  ...
  useEffect(() => {
    ...
    const client = new Client({
      ...
      onStompError: () => {
        // STOMP error — connection will attempt reconnect
      },
      reconnectDelay: 5000,
    });
  ...
  const handleSend = useCallback(
    (content: string) => {
      if (!stompRef.current?.connected) {
        // Fallback: optimistic update without STOMP
  ...
  /* ── Derived nickname for header ────────────────────────── */

  const headerTitle = room
    ? room.sellerNickname || room.buyerNickname || '채팅'
    : '채팅';
  ...
  const currentSenderId = -1; // placeholder; real app would use user id from auth
